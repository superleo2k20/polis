...
`;
  ...
  function geoCode(locationString) {
    return pgQueryP("select * from geolocation_cache where location = ($1);", [
      locationString,
    ]).then(function (rows) {
      if (!rows || !rows.length) {
  ...
  function doFamousQuery(o, req) {
    ...
    return Promise.all([
      ...
      return Promise.all([
        ...
        participantsWithSocialInfo = participantsWithSocialInfo.map(function (
          p
        ) {
